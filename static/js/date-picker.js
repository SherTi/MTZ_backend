function calendar(current = new Date()) {
  ...
  const lastDays =
    32 - new Date(current.getFullYear(), current.getMonth() - 1, 32).getDate();
  const daysInMoth =
    32 - new Date(current.getFullYear(), current.getMonth(), 32).getDate();
...
function createCalendar(
  ...
  span.innerHTML = selectedDateValue
  ...
  prevMonth.innerHTML =
    '<img src="/images/chevron-left.svg" class="picker-icon-img click">';
  ...
  nextMonth.innerHTML =
    '<img src="/images/chevron-right.svg" class="picker-icon-img click">';
  ...
  tr.innerHTML =
    "<td class='click'><div class='table-td click'>Пн</div></td>" +
  ...
  arr.calendar.map((week) => {
    ...
    week.map((day) => {
      ...
      if (
        new Date(arr.date.getFullYear(), arr.date.getMonth(), 1).getTime() ===
          new Date(
      ...
      const currentTimestamp = new Date(
        arr.date.getFullYear(),
        arr.date.getMonth(),
        day.value,
      ).getTime();
      if (
        currentTimestamp > options.min &&
        currentTimestamp < options.max &&
        day.current
      ) {
        div.addEventListener("click", () => {
          ...
          span.innerHTML = `${selected.getDate()} ${
